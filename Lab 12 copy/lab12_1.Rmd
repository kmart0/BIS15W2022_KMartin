...
```{r}
leftjoin_spiders_all <- left_join( spiders_locs, spiders_all, by="Accession")
leftjoin_spiders_all
```
...
```{r}
class(leftjoin_spiders_all$Date)
```
...
```{r}
dmy(leftjoin_spiders_all$Date)
```

If you want to use base R, then here is the format.
```{r}
as.Date(leftjoin_spiders_all$Date, format = "%d/%m/%y")
```
`
...
```{r}
spiders_with_locs <- leftjoin_spiders_all
```
