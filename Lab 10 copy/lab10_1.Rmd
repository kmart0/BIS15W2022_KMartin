...
```{r message=FALSE, warning=FALSE}
life_history <- read_csv(("data/mammal_lifehistories_v2.csv")) %>% na_if("-999") %>% clean_names()
```
...
```{r}
options(scipen=999) #cancels scientific notation for the session (ggplot wants to put things in scientific notation)
```
...
#If we wanted to make this best fit line a curve, how would we do that?
```
...
```
```{r}
elephants %>% ggplot(aes(x=sex))+ geom_bar()
```

2. How does the age of elephants compare by sex?
```{r}
elephants %>% ggplot(aes(x=sex, y=age)) +geom_boxplot()
```
```{r}
elephants %>% group_by(sex) %>% summarise(avg_age=mean(age))
```
...
```{r}
elephants %>% ggplot(aes(x=age, y=height))+geom_point()+ geom_smooth()
```
...
homerange %>% 
  ...
  labs(title="Log 10 Mass vs Homerange",
       x="log10.mass",
       y="mean.hra.m2")
```
