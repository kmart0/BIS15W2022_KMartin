...
venn_plot +
  scale_fill_distiller()+ #distiller is for discrete data
  labs(title = "Do all birds fly? Do only birds fly?",
         caption = ("you can add a caption here, e.g.: Studies show not all birds fly, and not only birds fly")
  )
```
...
swim_carn_aqua_ecto_vc <- list(swimming_vec, carni_vec, aqua_vec, ecto_vec)

# Make the plot:

ggVennDiagram(swim_carn_aqua_ecto_vc, category.names = c("Swimmers", "Carnivores", "Aquatics", "Ectotherms"))
```
