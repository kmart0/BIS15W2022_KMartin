...
```{r}
#I am curious about the bald and red eyed heroes that aren't evil, there's 8 total
superhero_info %>% 
  filter(hair_color=="No Hair", 
         eye_color=="red", 
         alignment!="bad") %>% 
  select(name, alignment, gender, hair_color, eye_color)
```
...
I want to look at the superhero powers data actually (hope that's ok), I want to see the different combinations of powers!
-  super strength?
-  super speed?
-  intelligence?
-  agility?
...
```{r}
superstrength_vec <- superhero_powers %>% 
  filter(super_strength=="TRUE") %>% 
  pull(hero_names)

superspeed_vec <- superhero_powers %>% 
  filter(super_speed=="TRUE") %>% 
  pull(hero_names)

intelligence_vec <- superhero_powers %>% 
  filter(intelligence=="TRUE") %>% 
  pull(hero_names)

agility_vec <- superhero_powers %>% 
  filter(agility=="TRUE") %>% 
  pull(hero_names)

power_combo_list <- list(superstrength_vec, superspeed_vec, intelligence_vec, agility_vec)

```

```{r}
ggVennDiagram(power_combo_list, category.names = c("Super Strength", "Super Speed", "Intelligence", "Agility"))
```
-  I think it's very interesting that about 20% of all of these heroes have all of the above *except* intelligence... But I want to see the heroes that have all of the above:
...
superhero_powers %>% 
  filter(super_strength=="TRUE", 
         ...
         intelligence=="TRUE", 
         agility=="TRUE") %>% 
  select(hero_names, super_strength, super_speed, intelligence, agility)
```

I also just want to know the super strong, fast, and smart heroes that don't have agility powers...
...
```
-  It's interesting Wonder Woman is on this list, I would say she definitely has agility abilities!
