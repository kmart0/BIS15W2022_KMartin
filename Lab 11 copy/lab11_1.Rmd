...
```{r message=FALSE, warning=FALSE}
deserts <- read_csv(here("Lab 11 copy", "data", "surveys_complete.csv"))
```
...
```{r message=FALSE, warning=FALSE}
homerange <- read_csv(here("Lab 11 copy", "data", "Tamburelloetal_HomeRangeDatabase.csv"))
```
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "darkseagreen", bins=40)+
...
homerange %>% 
  ...
  geom_histogram(fill="darkseagreen", color="black", alpha=0.5)+
  labs(title = "Log 10 Homerange")
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_density(color="darkblue", fill="steelblue")
  labs(title = "Log 10 Homerange")
```
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_histogram(aes(y = ..density..), fill="steelblue", color="darkblue", alpha=0.5)+
...
homerange %>% 
  ...
  labs(title="Observations of Range Category by Class")
```
