...
summary(amniota)
```
NA's are shown as `-999.0` :(


2. Use one of the tools explained above to summarize NA's in the data.
```{r}
#I can't because R doesn't know they are NA's so I have to transform them first
```
...
We need to replace the -999's with NA's so R knows they are NA's
...
```{r}
mam_life_hist2 <- readr::read_csv("data/mammal_lifehistories_v2.csv")
```
```{r}
summary(mam_life_hist2)
```
Yes, NA's are -999's
...
```{r}
better_mam_life_hist2 <- mam_life_hist2 %>% na_if("-999")
```

```{r}
any_na(better_mam_life_hist2)
```

```{r}
summary(better_mam_life_hist2)
```


4. In which variable are the most NAs in the data frame?
```{r}
better_mam_life_hist2 %>% 
  purrr::map_df(~ sum(is.na(.))) %>% 
  pivot_longer(everything(),
    names_to= "variables",
    values_to = "num_nas") %>% 
  arrange(desc(num_nas))
```
Weaning mass has the highest number of NA's
