...
It seems like amniota data has NA's represented as -999, and the amphibio data has actual NA's
...
## # ... with 26 more rows
```
This does not make sense, so we know we need to designate the -999's as NA's
...
This does make sense, so NA's are probably recognized by R.
...
```
For these columns, there are 1's denoting that the species is of this classification and an NA represents the species is not of this classification so it isn't actually an NA, its just not an affirmative value.
